...
- `scripts/develop` — creates `config/` (a scratch Home Assistant instance) if missing via `hass --script ensure_config`, adds `custom_components` to `PYTHONPATH`, then runs `hass --config ./config --debug`. This is how you run a live HA instance against your working copy of the integration without symlinks. Home Assistant will be reachable on port 8123.
